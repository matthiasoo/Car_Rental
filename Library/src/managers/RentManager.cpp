...
std::string RentManager::listRents(bool archive) {
    if (archive) return this->archiveRents->report();
...
RentPtr RentManager::rentVehicle(const int &id, ClientPtr client, VehiclePtr vehicle, pt::ptime &beginTime) {
    if (!client) throw NullPointerException("Client cannot be null!");
    if (!vehicle) throw NullPointerException("Vehicle cannot be null!");
    if (client->isArchive()) throw ClientException("Client account must be active!");
    if (vehicle->isArchive()) throw VehicleException("Vehicle must be available!");
    if (this->getAllClientRents(client).size() >= client->getMaxVehicles())
        throw RentException("Max number of rents achieved!");
    if (this->getVehicleRent(vehicle))
        throw VehicleException("Vehicle already rented!");
